...
def generate(
    ...
    for example in tqdm.tqdm(examples, desc=f'Generating {desc}'):
        ...
        while len(token_ids) <= model_max_length:
            ...
            # minitorch tensor does not support slicing, so convert to numpy
            # Get the token with the highest probability

            idx = minitorch.tensor_from_numpy(np.array([token_ids]), backend=backend)
            model_out = model(idx)
            last_token = model_out.to_numpy()[0, -1, :]
            gen_id = int(np.argmax(last_token))
            # END ASSIGN3_4

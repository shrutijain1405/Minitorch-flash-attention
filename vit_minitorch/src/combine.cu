...
__global__ void MatrixMultiplyKernel(
  ...
  for (int i = 0; i<num_tiles; i++){ 
    ...
    if (row < m && col < p){  
      for (int k=0; k<TILE; k++)
        ans = ans + a_shared[threadIdx.x][k] * b_shared[k][threadIdx.y];
    }
  ...
  if (row < m && col < p){ 
    out[batch*out_strides[0] + row*out_strides[1] + col*out_strides[2]] = ans;
  }
  /// END HW1_4
}
...
__global__ void reduceKernel(
  ...
  int input_idx[MAX_DIMS];
  for(int i = 0; i<shape_size; i++)
    input_idx[i] = out_index[i];

  double ans = reduce_value;
  for(int i = 0; i<a_shape[reduce_dim]; i++){
    input_idx[reduce_dim] = i;
    int in_pos = index_to_position(input_idx, a_strides, shape_size);
    ans = fn(fn_id, ans, a_storage[in_pos]);  
  }

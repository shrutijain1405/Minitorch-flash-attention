...
def binary_cross_entropy_loss(out, y):
    ...
    term1 = minitorch.Log.apply(out)
    term2 = minitorch.Log.apply(ones - out)
    loss = -1 * (y * term1 + (ones - y) * term2).mean()
    return loss
...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        ...
        out = minitorch.MatMul.apply(x, w)
...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        batch, sent_len, dim = embeddings.shape
        sentence = (embeddings.mean(dim=1)).view(batch, dim)
        out = self.linear1(sentence)
        out = out.relu()

        if self.training:
            out = minitorch.nn.dropout(out, self.dropout_prob)
    ...
    def train(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            if data_val is not None:
                ...
                out = model(x)
                validation_predictions += get_predictions_array(y_true=y, model_output=out)
